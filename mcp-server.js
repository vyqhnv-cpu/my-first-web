#!/usr/bin/env node
// mcp-server.js
// MCP Server for The Lifeskill Hub
// Connects goClaw Agent to the local SQLite diary and the cloud Supabase DB.
...
try {
  ...
  console.error("Warning: dotenv is not loaded or missing config");
}
...
// Dynamic import of the ESM MCP SDK inside async main
async function main() {
  ...
  server.registerTool(
    "sync_daily_diary",
    {
      description: "Làm sạch câu chữ trong nhật ký theo Brand Voice và lưu vào SQLite database + file Markdown data/cam_xuc.md.",
    ...
    async ({ raw_text }) => {
      try {
        ...
        const mdPath = path.resolve(__dirname, 'data', 'cam_xuc.md');
